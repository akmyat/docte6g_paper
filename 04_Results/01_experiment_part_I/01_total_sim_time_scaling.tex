...
In the stationary scenario, where stations are not mobile, \texttt{sionnart} backend is faster than the \texttt{pure\_ns-3} baseline once the station count reaches $N=2$, and at $N=32$ it is approximately 40 times faster than the baseline.
However, this result should not be interpreted as ray-tracing is faster than stochastic Friis propagation model as the reason why it is faster is because of the cache mechanisms.
With no mobility, no changes in positions happen so that the ray-tracer is invoked at the initial channel snapshot and the subsequent propagation queries are served by cached channel realizations.
In contrast, the pure ns-3 baseline still evaluates the analytical loss model for each packet-level query.
